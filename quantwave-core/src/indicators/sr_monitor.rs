...
pub struct SRInteractionMonitor {
    ...
    touch_tolerance: f64,
    approach_zone: f64,
    min_level_separation: f64, // to de-dup auto levels
    max_auto_levels: usize,    // safety bound for auto-generated levels

    // Active levels: key is stable id (we use a simple counter; user can also pass ids)
...
impl SRInteractionMonitor {
    ...
    fn maybe_add_auto_levels(&mut self, state: &MarketStructureState) {
        ...
        for sp in candidates {
            // De-dup: skip if too close to any existing level
            let too_close = self.levels.values().any(|l| {
                (l.price - sp.price).abs() < self.min_level_separation
            });
            if too_close {
                continue;
            }
...
impl Next<(f64, f64, f64)> for SRInteractionMonitor {
    ...
    fn next(&mut self, (high, low, close): (f64, f64, f64)) -> Self::Output {
        ...
        // Feed the foundation (swing detection + bias/flips)
        let structure = self.ms.next((high, low));

        // Promote any brand new swings into auto horizontal levels
        self.maybe_add_auto_levels(&structure);

        // Run detection across every active level (order not guaranteed; consumers sort by bar if needed)
        let mut all_interactions: Vec<SRInteraction> = Vec::new();

        // We collect keys first to avoid borrow issues while mutating inside loop
        let level_ids: Vec<u32> = self.levels.keys().copied().collect();

        let touch_tol = self.touch_tolerance;
        let appr_zone = self.approach_zone;
        for id in level_ids {
