...
use crate::indicators::patterns::candle_settings::{
    candle_color, lower_shadow, real_body, upper_shadow,
    CandleWindow, RollingCandleAvg,
    BODY_DOJI, BODY_LONG, BODY_SHORT, EQUAL, NEAR, SHADOW_VERY_SHORT
};
...
impl CDLGAPSIDESIDEWHITE {
    pub fn new() -> Self {
        Self { bars_seen: 0, window: CandleWindow::new(3), near_avg: RollingCandleAvg::new(NEAR) }
    }
}
...
impl Next<(f64, f64, f64, f64)> for CDLGAPSIDESIDEWHITE {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let near_same = (real_body(self.window.bar(1).open, self.window.bar(1).close)
            - real_body(self.window.bar(0).open, self.window.bar(0).close)).abs()
            < self.near_avg.val(1);

        let bull = real_body_gap_up(&self.window, 1, 2)
            && c1 == 1 && c0 == 1
            && self.window.bar(0).open < self.window.bar(1).close && self.window.bar(0).open > self.window.bar(1).open
...
impl Next<(f64, f64, f64, f64)> for CDLIDENTICAL3CROWS {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(2).open, self.window.bar(2).close) == -1
            ...
            && self.window.bar(0).close < self.window.bar(1).close
            && lower_shadow(self.window.bar(2).open, self.window.bar(2).low, self.window.bar(2).close) < self.shadow_avg.val(2)
            && lower_shadow(self.window.bar(1).open, self.window.bar(1).low, self.window.bar(1).close) < self.shadow_avg.val(1)
            && lower_shadow(self.window.bar(0).open, self.window.bar(0).low, self.window.bar(0).close) < self.shadow_avg.val(0)
...
impl Next<(f64, f64, f64, f64)> for CDLSTALLEDPATTERN {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(2).open, self.window.bar(2).close) == 1
            ...
            && upper_shadow(self.window.bar(1).open, self.window.bar(1).high, self.window.bar(1).close) < self.shadow_vs.val(1)
            && self.window.bar(1).open > self.window.bar(2).open
            && self.window.bar(1).open <= self.window.bar(2).close + self.near.val(2)
            && real_body(self.window.bar(0).open, self.window.bar(0).close) < self.body_short.val(0)
            && self.window.bar(0).open >= self.window.bar(1).close - real_body(self.window.bar(0).open, self.window.bar(0).close) - self.near.val(1);
...
impl Next<(f64, f64, f64, f64)> for CDLTRISTAR {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let base = real_body(self.window.bar(2).open, self.window.bar(2).close) <= self.body_doji.val(2)
            && real_body(self.window.bar(1).open, self.window.bar(1).close) <= self.body_doji.val(1)
            && real_body(self.window.bar(0).open, self.window.bar(0).close) <= self.body_doji.val(0);

        let bear = base && real_body_gap_up(&self.window, 1, 2)
            && !real_body_gap_up(&self.window, 0, 1);
        let bull = base && real_body_gap_down(&self.window, 1, 2)
...
impl Next<(f64, f64, f64, f64)> for CDLUNIQUE3RIVER {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(2).open, self.window.bar(2).close) == -1
            && real_body(self.window.bar(2).open, self.window.bar(2).close) > self.body_long.val(2)
            && candle_color(self.window.bar(1).open, self.window.bar(1).close) == -1
            && self.window.bar(1).open.max(self.window.bar(1).close) < self.window.bar(2).open.max(self.window.bar(2).close)
            && self.window.bar(1).open.min(self.window.bar(1).close) > self.window.bar(2).open.min(self.window.bar(2).close)
...
impl Next<(f64, f64, f64, f64)> for CDLUPSIDEGAP2CROWS {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(2).open, self.window.bar(2).close) == 1
            && real_body(self.window.bar(2).open, self.window.bar(2).close) > self.body_long.val(2)
            && candle_color(self.window.bar(1).open, self.window.bar(1).close) == -1
            && real_body(self.window.bar(1).open, self.window.bar(1).close) <= self.body_short.val(1)
...
impl Next<(f64, f64, f64, f64)> for CDLXSIDEGAP3METHODS {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let opens_within = self.window.bar(0).open > self.window.bar(1).open.min(self.window.bar(1).close)
            && self.window.bar(0).open < self.window.bar(1).open.max(self.window.bar(1).close);
        let closes_within = self.window.bar(0).close > self.window.bar(2).open.min(self.window.bar(2).close)
            && self.window.bar(0).close < self.window.bar(2).open.max(self.window.bar(2).close);
...
impl Next<(f64, f64, f64, f64)> for CDLCONCEALBABYSWALL {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(3).open, self.window.bar(3).close) == -1
            ...
            && candle_color(self.window.bar(1).open, self.window.bar(1).close) == -1
            && candle_color(self.window.bar(0).open, self.window.bar(0).close) == -1
            && lower_shadow(self.window.bar(3).open, self.window.bar(3).low, self.window.bar(3).close) == 0.0
            && upper_shadow(self.window.bar(3).open, self.window.bar(3).high, self.window.bar(3).close) == 0.0
            && lower_shadow(self.window.bar(2).open, self.window.bar(2).low, self.window.bar(2).close) == 0.0
            && upper_shadow(self.window.bar(2).open, self.window.bar(2).high, self.window.bar(2).close) == 0.0
            && real_body_gap_down(&self.window, 2, 3)
            && lower_shadow(self.window.bar(1).open, self.window.bar(1).low, self.window.bar(1).close) > self.shadow_vs.val(1)
...
impl Next<(f64, f64, f64, f64)> for CDLLADDERBOTTOM {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let cond = candle_color(self.window.bar(4).open, self.window.bar(4).close) == -1
            ...
            && candle_color(self.window.bar(2).open, self.window.bar(2).close) == -1
            && candle_color(self.window.bar(1).open, self.window.bar(1).close) == -1
            && self.window.bar(3).close < self.window.bar(4).close && self.window.bar(2).close < self.window.bar(3).close
            && upper_shadow(self.window.bar(1).open, self.window.bar(1).high, self.window.bar(1).close) > self.shadow_vs.val(1)
            && candle_color(self.window.bar(0).open, self.window.bar(0).close) == 1
            && self.window.bar(0).open > self.window.bar(1).open
...
impl Next<(f64, f64, f64, f64)> for CDLMATHOLD {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let penetration = 0.5;
        let cond = real_body(self.window.bar(4).open, self.window.bar(4).close) > self.body_long.val(4)
            && real_body(self.window.bar(3).open, self.window.bar(3).close) < self.body_short.val(3)
            && real_body(self.window.bar(2).open, self.window.bar(2).close) < self.body_short.val(2)
            && real_body(self.window.bar(1).open, self.window.bar(1).close) < self.body_short.val(1)
            ...
            && self.window.bar(2).open.min(self.window.bar(2).close) > self.window.bar(4).close - real_body(self.window.bar(4).open, self.window.bar(4).close) * penetration
            && self.window.bar(1).open.min(self.window.bar(1).close) > self.window.bar(4).close - real_body(self.window.bar(4).open, self.window.bar(4).close) * penetration
            && self.window.bar(2).open.max(self.window.bar(2).close) < self.window.bar(3).open
            && self.window.bar(1).open.max(self.window.bar(1).close) < self.window.bar(2).open.max(self.window.bar(2).close)
            && self.window.bar(0).open > self.window.bar(1).close
            && self.window.bar(0).close > self.window.bar(3).high.max(self.window.bar(2).high).max(self.window.bar(1).high);
...
impl Next<(f64, f64, f64, f64)> for CDLRISEFALL3METHODS {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        if real_body(self.window.bar(4).open, self.window.bar(4).close) > self.body_long.val(4)
            && real_body(self.window.bar(0).open, self.window.bar(0).close) > self.body_long.val(0)
        {
            let mid_short = real_body(self.window.bar(3).open, self.window.bar(3).close) < self.body_short.val(3)
                && real_body(self.window.bar(2).open, self.window.bar(2).close) < self.body_short.val(2)
                && real_body(self.window.bar(1).open, self.window.bar(1).close) < self.body_short.val(1);
            ...
            let bull = c4 == 1 && mid_short
                ...
                && self.window.bar(2).close < self.window.bar(3).close
                && self.window.bar(1).close < self.window.bar(2).close
                && self.window.bar(3).low > self.window.bar(4).low && self.window.bar(2).low > self.window.bar(4).low && self.window.bar(1).low > self.window.bar(4).low
                && self.window.bar(3).high < self.window.bar(4).high && self.window.bar(2).high < self.window.bar(4).high && self.window.bar(1).high < self.window.bar(4).high
                && c0 == 1
                && self.window.bar(0).open > self.window.bar(1).close
...
mod tests {
    use super::*;
    crate::test_pattern_parity!(test_cdlgapsidesidewhite, CDLGAPSIDESIDEWHITE, talib_rs::pattern::cdl_gapsidesidewhite);
    crate::test_pattern_parity!(test_cdlidentical3crows, CDLIDENTICAL3CROWS, talib_rs::pattern::cdl_identical3crows);
    crate::test_pattern_parity!(test_cdlstalledpattern, CDLSTALLEDPATTERN, talib_rs::pattern::cdl_stalledpattern);
    crate::test_pattern_parity!(test_cdltasukigap, CDLTASUKIGAP, talib_rs::pattern::cdl_tasukigap);
    crate::test_pattern_parity!(test_cdltristar, CDLTRISTAR, talib_rs::pattern::cdl_tristar);
    crate::test_pattern_parity!(test_cdlunique3river, CDLUNIQUE3RIVER, talib_rs::pattern::cdl_unique3river);
    crate::test_pattern_parity!(test_cdlupsidegap2crows, CDLUPSIDEGAP2CROWS, talib_rs::pattern::cdl_upsidegap2crows);
    crate::test_pattern_parity!(test_cdlxsidegap3methods, CDLXSIDEGAP3METHODS, talib_rs::pattern::cdl_xsidegap3methods);
    crate::test_pattern_parity!(test_cdlbreakaway, CDLBREAKAWAY, talib_rs::pattern::cdl_breakaway);
    crate::test_pattern_parity!(test_cdlconcealbabyswall, CDLCONCEALBABYSWALL, talib_rs::pattern::cdl_concealbabyswall);
    crate::test_pattern_parity!(test_cdlladderbottom, CDLLADDERBOTTOM, talib_rs::pattern::cdl_ladderbottom);
    crate::test_pattern_parity!(test_cdlmathold, CDLMATHOLD, talib_rs::pattern::cdl_mathold);
    crate::test_pattern_parity!(test_cdlrisefall3methods, CDLRISEFALL3METHODS, talib_rs::pattern::cdl_risefall3methods);
}
