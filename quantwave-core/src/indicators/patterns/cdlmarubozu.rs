//! Native O(1) streaming CDLMARUBOZU (matches `talib_rs::pattern::cdl_marubozu`).
//! Source: TA-Lib
...
impl Next<(f64, f64, f64, f64)> for CDLMARUBOZU {
    ...
    fn next(&mut self, (open, high, low, close): (f64, f64, f64, f64)) -> Self::Output {
        ...
        let out = (real_body(curr.open, curr.close) > self.body_long.val(0)
            ...
            && lower_shadow(curr.open, curr.low, curr.close) < self.shadow_vs.val(0)) as i32 * candle_color(curr.open, curr.close) * 100;
...
mod tests {
    use super::*;
    crate::test_pattern_parity!(test_cdlmarubozu_parity, CDLMARUBOZU, talib_rs::pattern::cdl_marubozu);
}
