...
impl Next<f64> for MAMA {
    ...
    fn next(&mut self, price: f64) -> Self::Output {
        ...
        let detrender = (0.0962 * self.smooth_history[0] 
                       + 0.5769 * self.smooth_history[2] 
                       - 0.5769 * self.smooth_history[4] 
                       - 0.0962 * self.smooth_history[6]) * (0.075 * self.period_prev + 0.54);
        ...
        let q1 = (0.0962 * self.detrender_history[0] 
                + 0.5769 * self.detrender_history[2] 
                - 0.5769 * self.detrender_history[4] 
                - 0.0962 * self.detrender_history[6]) * (0.075 * self.period_prev + 0.54);
        ...
        let ji = (0.0962 * self.i1_history[0] 
                + 0.5769 * self.i1_history[2] 
                - 0.5769 * self.i1_history[4] 
                - 0.0962 * self.i1_history[6]) * (0.075 * self.period_prev + 0.54);

        // jQ = (.0962*Q1 + .5769*Q1[2] - .5769*Q1[4] - .0962*Q1[6])*(.075*Period[1] + .54);
        let jq = (0.0962 * self.q1_history[0] 
                + 0.5769 * self.q1_history[2] 
                - 0.5769 * self.q1_history[4] 
                - 0.0962 * self.q1_history[6]) * (0.075 * self.period_prev + 0.54);
...
pub const MAMA_METADATA: IndicatorMetadata = IndicatorMetadata {
    ...
    params: &[
        ParamDef { name: "fast_limit", default: "0.5", description: "Fast limit for alpha" },
        ParamDef { name: "slow_limit", default: "0.05", description: "Slow limit for alpha" },
    ],
