...
def collect_entries() -> list[tuple[str, str]]:
    ...
    for path in sorted(INDICATORS_DIR.rglob("*.rs")):
        ...
        for const_name in CONST_RE.findall(text):
            entries.append((module_path(path), const_name))
    # stable dedupe by const name
    seen: set[str] = set()
    unique: list[tuple[str, str]] = []
    for mod_path, const_name in entries:
        if const_name in seen:
            continue
        seen.add(const_name)
        unique.append((mod_path, const_name))
...
def render(entries: list[tuple[str, str]]) -> str:
    use_lines = sorted({f"use {mod_path}::{const_name};" for mod_path, const_name in entries})
    registered = "\n    ".join(
        f'RegisteredMetadata {{ slug: "{slug_from_const(c)}", meta: &{c} }},'
        for _, c in entries
    )
    refs = "\n    ".join(f"&{const_name}," for _, const_name in entries)
