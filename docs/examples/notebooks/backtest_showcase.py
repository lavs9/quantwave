...
def __(mo):
    mo.md(
        ...
        "See `quantwave_backtest::monte_carlo_trade_bootstrap` and "
        "`monte_carlo_return_paths` in Rust for bootstrap and return-path VaR/CVaR."
    )
    return
...
def __(mo):
    mo.md(
        ...
        "QuantWave guarantees exact parity between streaming and batch mode. "
        "See [Batch & Streaming](../batch-streaming.md) and "
        "[ML Feature Parity](ml_feature_backtest_parity.md)."
    )
    return
