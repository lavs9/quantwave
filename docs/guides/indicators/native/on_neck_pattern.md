...
> **Chart**: Annotated candlestick diagram for **On-Neck Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLONNECK` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLONNECK;
use quantwave_core::traits::Next;

let mut det = CDLONNECK::new();
...
```python
from quantwave import CDLONNECK
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_onneck("open", "high", "low", "close").alias("on_neck_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLONNECK` / `CDLONNECK_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlonneck.json`
