...
> **Chart**: Annotated candlestick diagram for **Spinning Top** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLSPINNINGTOP` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLSPINNINGTOP;
use quantwave_core::traits::Next;

let mut det = CDLSPINNINGTOP::new();
...
```python
from quantwave import CDLSPINNINGTOP
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_spinningtop("open", "high", "low", "close").alias("spinning_top")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLSPINNINGTOP` / `CDLSPINNINGTOP_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlspinningtop.json`
