...
> **Chart**: Annotated candlestick diagram for **Marubozu** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLMARUBOZU` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLMARUBOZU;
use quantwave_core::traits::Next;

let mut det = CDLMARUBOZU::new();
...
```python
from quantwave import CDLMARUBOZU
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_marubozu("open", "high", "low", "close").alias("marubozu")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLMARUBOZU` / `CDLMARUBOZU_METADATA`).
