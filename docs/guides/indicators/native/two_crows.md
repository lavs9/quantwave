...
> **Chart**: Annotated candlestick diagram for **Two Crows** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDL2CROWS` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDL2CROWS;
use quantwave_core::traits::Next;

let mut det = CDL2CROWS::new();
...
```python
from quantwave import CDL2CROWS
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_2crows("open", "high", "low", "close").alias("two_crows")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDL2CROWS` / `CDL2CROWS_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdl2crows.json`
