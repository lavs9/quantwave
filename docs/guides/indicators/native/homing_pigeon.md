...
> **Chart**: Annotated candlestick diagram for **Homing Pigeon** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLHOMINGPIGEON` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLHOMINGPIGEON;
use quantwave_core::traits::Next;

let mut det = CDLHOMINGPIGEON::new();
...
```python
from quantwave import CDLHOMINGPIGEON
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_homingpigeon("open", "high", "low", "close").alias("homing_pigeon")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLHOMINGPIGEON` / `CDLHOMINGPIGEON_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlhomingpigeon.json`
