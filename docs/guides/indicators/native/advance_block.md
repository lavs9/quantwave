...
> **Chart**: Annotated candlestick diagram for **Advance Block** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLADVANCEBLOCK` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLADVANCEBLOCK;
use quantwave_core::traits::Next;

let mut det = CDLADVANCEBLOCK::new();
...
```python
from quantwave import CDLADVANCEBLOCK
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_advanceblock("open", "high", "low", "close").alias("advance_block")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLADVANCEBLOCK` / `CDLADVANCEBLOCK_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdladvanceblock.json`
