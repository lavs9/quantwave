...
> **Chart**: Annotated candlestick diagram for **Stick Sandwich** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLSTICKSANDWICH` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLSTICKSANDWICH;
use quantwave_core::traits::Next;

let mut det = CDLSTICKSANDWICH::new();
...
```python
from quantwave import CDLSTICKSANDWICH

det = CDLSTICKSANDWICH()
for o, h, l, c in ohlcv:
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_sticksandwich("open", "high", "low", "close").alias("stick_sandwich")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLSTICKSANDWICH` / `CDLSTICKSANDWICH_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlsticksandwich.json`
