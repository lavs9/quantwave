...
> **Chart**: Sparkline or annotated price series showing **Triple Exponential Moving Average** behaviour on synthetic trending + cyclic data. Run `python docs/gen_indicator_previews.py --only triple_exponential_moving_average` after extending the generator.

*Visual placeholder — standards bulk upgrade 2026-06-25 IST. Core logic in `quantwave-core/src/indicators/tema.rs`.*
...
Gold-standard parity vectors: `quantwave-core/tests/gold_standard/tema.json`.
...
def apply_triple_exponential_moving_average(series: pl.Series) -> pl.Series:
    ind = qw.TEMA(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_triple_exponential_moving_average, return_dtype=pl.Float64).alias("triple_exponential_moving_average")
    )
    .collect()
)
```
...
**Primary Source**: https://www.investopedia.com/terms/t/triple-exponential-moving-average.asp
