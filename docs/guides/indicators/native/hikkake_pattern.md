...
> **Chart**: Annotated candlestick diagram for **Hikkake Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLHIKKAKE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLHIKKAKE;
use quantwave_core::traits::Next;

let mut det = CDLHIKKAKE::new();
...
```python
from quantwave import CDLHIKKAKE
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_hikkake("open", "high", "low", "close").alias("hikkake_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLHIKKAKE` / `CDLHIKKAKE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlhikkake.json`
