...
> **Chart**: Annotated candlestick diagram for **Unique 3 River** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLUNIQUE3RIVER` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLUNIQUE3RIVER;
use quantwave_core::traits::Next;

let mut det = CDLUNIQUE3RIVER::new();
...
```python
from quantwave import CDLUNIQUE3RIVER
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_unique3river("open", "high", "low", "close").alias("unique_3_river")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLUNIQUE3RIVER` / `CDLUNIQUE3RIVER_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlunique3river.json`
