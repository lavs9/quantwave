...
The Breakaway indicator is a technical analysis tool that a five-candle reversal pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLBREAKAWAY` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLBREAKAWAY;
use quantwave_core::traits::Next;

let mut det = CDLBREAKAWAY::new();
...
```python
from quantwave import CDLBREAKAWAY
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_breakaway("open", "high", "low", "close").alias("breakaway")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLBREAKAWAY` / `CDLBREAKAWAY_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlbreakaway.json`
