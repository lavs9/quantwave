...
![Three-Line Strike — annotated preview mapping to core implementation](../../../assets/candlestick-previews/three_line_strike.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Three-Line Strike indicator is a technical analysis tool that a continuation pattern where four candles are involved.
...
**Recognition Rules (TA-Lib-compatible, `CDL3LINESTRIKE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDL3LINESTRIKE;
use quantwave_core::traits::Next;

let mut det = CDL3LINESTRIKE::new();
...
```python
from quantwave import CDL3LINESTRIKE
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_3linestrike("open", "high", "low", "close").alias("three_line_strike")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDL3LINESTRIKE` / `CDL3LINESTRIKE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdl3linestrike.json`
