...
> **Chart**: Annotated candlestick diagram for **Morning Doji Star** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLMORNINGDOJISTAR` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLMORNINGDOJISTAR;
use quantwave_core::traits::Next;

let mut det = CDLMORNINGDOJISTAR::new();
...
```python
from quantwave import CDLMORNINGDOJISTAR

det = CDLMORNINGDOJISTAR()
for o, h, l, c in ohlcv:
...
def apply_morning_doji_star(series: pl.Series) -> pl.Series:
    ind = qw.CDLMORNINGDOJISTAR(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_morning_doji_star, return_dtype=pl.Float64).alias("morning_doji_star")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLMORNINGDOJISTAR` / `CDLMORNINGDOJISTAR_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlmorningdojistar.json`
