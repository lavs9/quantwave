...
![High-Wave Candle — annotated preview mapping to core implementation](../../../assets/candlestick-previews/high_wave_candle.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The High-Wave Candle indicator is a technical analysis tool that a candle with very long shadows and a small body.
...
**Recognition Rules (TA-Lib-compatible, `CDLHIGHWAVE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLHIGHWAVE;
use quantwave_core::traits::Next;

let mut det = CDLHIGHWAVE::new();
...
```python
from quantwave import CDLHIGHWAVE
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_highwave("open", "high", "low", "close").alias("high_wave_candle")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLHIGHWAVE` / `CDLHIGHWAVE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlhighwave.json`
