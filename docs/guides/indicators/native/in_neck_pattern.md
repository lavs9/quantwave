...
> **Chart**: Annotated candlestick diagram for **In-Neck Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLINNECK` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLINNECK;
use quantwave_core::traits::Next;

let mut det = CDLINNECK::new();
...
```python
from quantwave import CDLINNECK
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_inneck("open", "high", "low", "close").alias("in_neck_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLINNECK` / `CDLINNECK_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlinneck.json`
