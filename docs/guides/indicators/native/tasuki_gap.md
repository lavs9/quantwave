...
> **Chart**: Annotated candlestick diagram for **Tasuki Gap** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLTASUKIGAP` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLTASUKIGAP;
use quantwave_core::traits::Next;

let mut det = CDLTASUKIGAP::new();
...
```python
from quantwave import CDLTASUKIGAP
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_tasukigap("open", "high", "low", "close").alias("tasuki_gap")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLTASUKIGAP` / `CDLTASUKIGAP_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdltasukigap.json`
