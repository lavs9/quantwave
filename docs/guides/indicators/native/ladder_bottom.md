...
> **Chart**: Annotated candlestick diagram for **Ladder Bottom** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLLADDERBOTTOM` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLLADDERBOTTOM;
use quantwave_core::traits::Next;

let mut det = CDLLADDERBOTTOM::new();
...
```python
from quantwave import CDLLADDERBOTTOM
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_ladderbottom("open", "high", "low", "close").alias("ladder_bottom")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLLADDERBOTTOM` / `CDLLADDERBOTTOM_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlladderbottom.json`
