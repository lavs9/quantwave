...
> **Chart**: Annotated candlestick diagram for **Matching Low** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLMATCHINGLOW` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLMATCHINGLOW;
use quantwave_core::traits::Next;

let mut det = CDLMATCHINGLOW::new();
...
```python
from quantwave import CDLMATCHINGLOW
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_matchinglow("open", "high", "low", "close").alias("matching_low")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLMATCHINGLOW` / `CDLMATCHINGLOW_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlmatchinglow.json`
