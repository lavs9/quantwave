...
> **Chart**: Annotated candlestick diagram for **Upside Gap Two Crows** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLUPSIDEGAP2CROWS` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLUPSIDEGAP2CROWS;
use quantwave_core::traits::Next;

let mut det = CDLUPSIDEGAP2CROWS::new();
...
```python
from quantwave import CDLUPSIDEGAP2CROWS

det = CDLUPSIDEGAP2CROWS()
for o, h, l, c in ohlcv:
...
def apply_upside_gap_two_crows(series: pl.Series) -> pl.Series:
    ind = qw.CDLUPSIDEGAP2CROWS(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_upside_gap_two_crows, return_dtype=pl.Float64).alias("upside_gap_two_crows")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLUPSIDEGAP2CROWS` / `CDLUPSIDEGAP2CROWS_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlupsidegap2crows.json`
