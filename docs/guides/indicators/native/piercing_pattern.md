...
> **Chart**: Annotated candlestick diagram for **Piercing Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLPIERCING` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLPIERCING;
use quantwave_core::traits::Next;

let mut det = CDLPIERCING::new();
...
```python
from quantwave import CDLPIERCING
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_piercing("open", "high", "low", "close").alias("piercing_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLPIERCING` / `CDLPIERCING_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlpiercing.json`
