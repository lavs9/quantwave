...
> **Chart**: Annotated candlestick diagram for **Kicking** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLKICKING` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLKICKING;
use quantwave_core::traits::Next;

let mut det = CDLKICKING::new();
...
```python
from quantwave import CDLKICKING
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_kicking("open", "high", "low", "close").alias("kicking")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLKICKING` / `CDLKICKING_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlkicking.json`
