...
> **Chart**: Annotated candlestick diagram for **Hanging Man** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLHANGINGMAN` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLHANGINGMAN;
use quantwave_core::traits::Next;

let mut det = CDLHANGINGMAN::new();
...
```python
from quantwave import CDLHANGINGMAN
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_hangingman("open", "high", "low", "close").alias("hanging_man")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLHANGINGMAN` / `CDLHANGINGMAN_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlhangingman.json`
