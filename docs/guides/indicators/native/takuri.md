...
> **Chart**: Annotated candlestick diagram for **Takuri** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLTAKURI` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLTAKURI;
use quantwave_core::traits::Next;

let mut det = CDLTAKURI::new();
...
```python
from quantwave import CDLTAKURI
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_takuri("open", "high", "low", "close").alias("takuri")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLTAKURI` / `CDLTAKURI_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdltakuri.json`
