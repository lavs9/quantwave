...
> **Chart**: Annotated candlestick diagram for **Up/Down-Gap Three Methods** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLXSIDEGAP3METHODS` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLXSIDEGAP3METHODS;
use quantwave_core::traits::Next;

let mut det = CDLXSIDEGAP3METHODS::new();
...
```python
from quantwave import CDLXSIDEGAP3METHODS

det = CDLXSIDEGAP3METHODS()
for o, h, l, c in ohlcv:
...
def apply_up_down_gap_three_methods(series: pl.Series) -> pl.Series:
    ind = qw.CDLXSIDEGAP3METHODS(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_up_down_gap_three_methods, return_dtype=pl.Float64).alias("up_down_gap_three_methods")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLXSIDEGAP3METHODS` / `CDLXSIDEGAP3METHODS_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlxsidegap3methods.json`
