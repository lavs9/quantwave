...
> **Chart**: Annotated candlestick diagram for **Mat Hold** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLMATHOLD` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLMATHOLD;
use quantwave_core::traits::Next;

let mut det = CDLMATHOLD::new();
...
```python
from quantwave import CDLMATHOLD
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_mathold("open", "high", "low", "close").alias("mat_hold")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLMATHOLD` / `CDLMATHOLD_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlmathold.json`
