...
![Rickshaw Man — annotated preview mapping to core implementation](../../../assets/candlestick-previews/rickshaw_man.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Rickshaw Man indicator is a technical analysis tool that a long-legged doji where the body is in the center of the range.
...
**Recognition Rules (TA-Lib-compatible, `CDLRICKSHAWMAN` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLRICKSHAWMAN;
use quantwave_core::traits::Next;

let mut det = CDLRICKSHAWMAN::new();
...
```python
from quantwave import CDLRICKSHAWMAN
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_rickshawman("open", "high", "low", "close").alias("rickshaw_man")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLRICKSHAWMAN` / `CDLRICKSHAWMAN_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlrickshawman.json`
