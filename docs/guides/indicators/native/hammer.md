...
![Hammer — annotated preview mapping to core implementation](../../../assets/candlestick-previews/hammer.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Hammer indicator is a technical analysis tool that a bullish reversal pattern with a small body and long lower shadow.
...
**Recognition Rules (TA-Lib-compatible, `CDLHAMMER` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLHAMMER;
use quantwave_core::traits::Next;

let mut det = CDLHAMMER::new();
...
```python
from quantwave import CDLHAMMER
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_hammer("open", "high", "low", "close").alias("hammer")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLHAMMER` / `CDLHAMMER_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlhammer.json`
