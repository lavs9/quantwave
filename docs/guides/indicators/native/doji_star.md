...
> **Chart**: Annotated candlestick diagram for **Doji Star** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLDOJISTAR` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLDOJISTAR;
use quantwave_core::traits::Next;

let mut det = CDLDOJISTAR::new();
...
```python
from quantwave import CDLDOJISTAR
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_dojistar("open", "high", "low", "close").alias("doji_star")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLDOJISTAR` / `CDLDOJISTAR_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdldojistar.json`
