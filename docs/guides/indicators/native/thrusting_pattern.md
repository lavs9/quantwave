...
> **Chart**: Annotated candlestick diagram for **Thrusting Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLTHRUSTING` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLTHRUSTING;
use quantwave_core::traits::Next;

let mut det = CDLTHRUSTING::new();
...
```python
from quantwave import CDLTHRUSTING
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_thrusting("open", "high", "low", "close").alias("thrusting_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLTHRUSTING` / `CDLTHRUSTING_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlthrusting.json`
