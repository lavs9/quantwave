...
> **Chart**: Annotated candlestick diagram for **Concealed Baby Swallow** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLCONCEALBABYSWALL` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLCONCEALBABYSWALL;
use quantwave_core::traits::Next;

let mut det = CDLCONCEALBABYSWALL::new();
...
```python
from quantwave import CDLCONCEALBABYSWALL

det = CDLCONCEALBABYSWALL()
for o, h, l, c in ohlcv:
...
def apply_concealed_baby_swallow(series: pl.Series) -> pl.Series:
    ind = qw.CDLCONCEALBABYSWALL(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_concealed_baby_swallow, return_dtype=pl.Float64).alias("concealed_baby_swallow")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLCONCEALBABYSWALL` / `CDLCONCEALBABYSWALL_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlconcealbabyswall.json`
