...
![Separating Lines — annotated preview mapping to core implementation](../../../assets/candlestick-previews/separating_lines.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Separating Lines indicator is a technical analysis tool that a continuation pattern where candles move in opposite directions.
...
**Recognition Rules (TA-Lib-compatible, `CDLSEPARATINGLINES` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLSEPARATINGLINES;
use quantwave_core::traits::Next;

let mut det = CDLSEPARATINGLINES::new();
...
```python
from quantwave import CDLSEPARATINGLINES

det = CDLSEPARATINGLINES()
for o, h, l, c in ohlcv:
...
def apply_separating_lines(series: pl.Series) -> pl.Series:
    ind = qw.CDLSEPARATINGLINES(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_separating_lines, return_dtype=pl.Float64).alias("separating_lines")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLSEPARATINGLINES` / `CDLSEPARATINGLINES_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlseparatinglines.json`
