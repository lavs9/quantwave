...
![Short Line Candle — annotated preview mapping to core implementation](../../../assets/candlestick-previews/short_line_candle.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Short Line Candle indicator is a technical analysis tool that a candle with a small body and small shadows.
...
**Recognition Rules (TA-Lib-compatible, `CDLSHORTLINE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLSHORTLINE;
use quantwave_core::traits::Next;

let mut det = CDLSHORTLINE::new();
...
```python
from quantwave import CDLSHORTLINE
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_shortline("open", "high", "low", "close").alias("short_line_candle")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLSHORTLINE` / `CDLSHORTLINE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlshortline.json`
