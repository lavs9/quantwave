...
> **Chart**: Annotated candlestick diagram for **Stalled Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLSTALLEDPATTERN` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLSTALLEDPATTERN;
use quantwave_core::traits::Next;

let mut det = CDLSTALLEDPATTERN::new();
...
```python
from quantwave import CDLSTALLEDPATTERN

det = CDLSTALLEDPATTERN()
for o, h, l, c in ohlcv:
...
def apply_stalled_pattern(series: pl.Series) -> pl.Series:
    ind = qw.CDLSTALLEDPATTERN(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_stalled_pattern, return_dtype=pl.Float64).alias("stalled_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLSTALLEDPATTERN` / `CDLSTALLEDPATTERN_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlstalledpattern.json`
