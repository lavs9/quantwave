...
> **Chart**: Sparkline or annotated price series showing **Simple Moving Average** behaviour on synthetic trending + cyclic data. Run `python docs/gen_indicator_previews.py --only simple_moving_average` after extending the generator.
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").ta.sma("close", 14).alias("simple_moving_average")
    )
    .collect()
)
```
