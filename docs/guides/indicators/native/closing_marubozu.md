...
> **Chart**: Annotated candlestick diagram for **Closing Marubozu** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLCLOSINGMARUBOZU` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLCLOSINGMARUBOZU;
use quantwave_core::traits::Next;

let mut det = CDLCLOSINGMARUBOZU::new();
...
```python
from quantwave import CDLCLOSINGMARUBOZU

det = CDLCLOSINGMARUBOZU()
for o, h, l, c in ohlcv:
...
def apply_closing_marubozu(series: pl.Series) -> pl.Series:
    ind = qw.CDLCLOSINGMARUBOZU(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_closing_marubozu, return_dtype=pl.Float64).alias("closing_marubozu")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLCLOSINGMARUBOZU` / `CDLCLOSINGMARUBOZU_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlclosingmarubozu.json`
