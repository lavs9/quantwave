...
> **Chart**: Annotated candlestick diagram for **Belt-Hold** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLBELTHOLD` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLBELTHOLD;
use quantwave_core::traits::Next;

let mut det = CDLBELTHOLD::new();
...
```python
from quantwave import CDLBELTHOLD
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_belthold("open", "high", "low", "close").alias("belt_hold")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLBELTHOLD` / `CDLBELTHOLD_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlbelthold.json`
