...
![Three Outside Up/Down — annotated preview mapping to core implementation](../../../assets/candlestick-previews/three_outside_up_down.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Three Outside Up/Down indicator is a technical analysis tool that a reversal pattern confirmed by a third candle.
...
**Recognition Rules (TA-Lib-compatible, `CDL3OUTSIDE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDL3OUTSIDE;
use quantwave_core::traits::Next;

let mut det = CDL3OUTSIDE::new();
...
```python
from quantwave import CDL3OUTSIDE
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_3outside("open", "high", "low", "close").alias("three_outside_up_down")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDL3OUTSIDE` / `CDL3OUTSIDE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdl3outside.json`
