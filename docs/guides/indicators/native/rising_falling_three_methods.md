...
![Rising/Falling Three Methods — annotated preview mapping to core implementation](../../../assets/candlestick-previews/rising_falling_three_methods.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Rising/Falling Three Methods indicator is a technical analysis tool that a strong continuation pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLRISEFALL3METHODS` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLRISEFALL3METHODS;
use quantwave_core::traits::Next;

let mut det = CDLRISEFALL3METHODS::new();
...
```python
from quantwave import CDLRISEFALL3METHODS

det = CDLRISEFALL3METHODS()
for o, h, l, c in ohlcv:
...
def apply_rising_falling_three_methods(series: pl.Series) -> pl.Series:
    ind = qw.CDLRISEFALL3METHODS(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_rising_falling_three_methods, return_dtype=pl.Float64).alias("rising_falling_three_methods")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLRISEFALL3METHODS` / `CDLRISEFALL3METHODS_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlrisefall3methods.json`
