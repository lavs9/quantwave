...
> **Chart**: Annotated candlestick diagram for **Tristar Pattern** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLTRISTAR` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLTRISTAR;
use quantwave_core::traits::Next;

let mut det = CDLTRISTAR::new();
...
```python
from quantwave import CDLTRISTAR
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_tristar("open", "high", "low", "close").alias("tristar_pattern")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLTRISTAR` / `CDLTRISTAR_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdltristar.json`
