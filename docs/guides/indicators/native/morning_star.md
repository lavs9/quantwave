...
The Morning Star indicator is a technical analysis tool that a bullish reversal pattern with three candles.
...
**Recognition Rules (TA-Lib-compatible, `CDLMORNINGSTAR` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLMORNINGSTAR;
use quantwave_core::traits::Next;

let mut det = CDLMORNINGSTAR::new();
...
```python
from quantwave import CDLMORNINGSTAR
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_morningstar("open", "high", "low", "close").alias("morning_star")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLMORNINGSTAR` / `CDLMORNINGSTAR_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlmorningstar.json`
