...
> **Chart**: Annotated candlestick diagram for **Dark Cloud Cover** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLDARKCLOUDCOVER` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLDARKCLOUDCOVER;
use quantwave_core::traits::Next;

let mut det = CDLDARKCLOUDCOVER::new();
...
```python
from quantwave import CDLDARKCLOUDCOVER

det = CDLDARKCLOUDCOVER()
for o, h, l, c in ohlcv:
...
def apply_dark_cloud_cover(series: pl.Series) -> pl.Series:
    ind = qw.CDLDARKCLOUDCOVER(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_dark_cloud_cover, return_dtype=pl.Float64).alias("dark_cloud_cover")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLDARKCLOUDCOVER` / `CDLDARKCLOUDCOVER_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdldarkcloudcover.json`
