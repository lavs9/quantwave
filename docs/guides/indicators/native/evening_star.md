...
> **Chart**: Annotated candlestick diagram for **Evening Star** showing the ideal TA-Lib recognition geometry (body/wick relationships and trend context). Extend `docs/gen_candle_previews.py` to add a dedicated generator for this pattern.
...
**Recognition Rules (TA-Lib-compatible, `CDLEVENINGSTAR` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLEVENINGSTAR;
use quantwave_core::traits::Next;

let mut det = CDLEVENINGSTAR::new();
...
```python
from quantwave import CDLEVENINGSTAR
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("open").ta.cdl_eveningstar("open", "high", "low", "close").alias("evening_star")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLEVENINGSTAR` / `CDLEVENINGSTAR_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdleveningstar.json`
