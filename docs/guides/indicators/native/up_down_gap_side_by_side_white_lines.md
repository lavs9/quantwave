...
![Up/Down-Gap Side-By-Side White Lines — annotated preview mapping to core implementation](../../../assets/candlestick-previews/up_down_gap_side_by_side_white_lines.png)

*Synthetic ideal per library logic. Generated 2026-06-25 IST via `docs/generate_all_previews.py` (reproducible; maps to core `Next<T>` implementation).*

## Description

The Up/Down-Gap Side-By-Side White Lines indicator is a technical analysis tool that a continuation pattern involving gaps.
...
**Recognition Rules (TA-Lib-compatible, `CDLGAPSIDESIDEWHITE` in `quantwave-core/src/indicators/pattern.rs`)**:
...
```rust
use quantwave_core::indicators::CDLGAPSIDESIDEWHITE;
use quantwave_core::traits::Next;

let mut det = CDLGAPSIDESIDEWHITE::new();
...
```python
from quantwave import CDLGAPSIDESIDEWHITE

det = CDLGAPSIDESIDEWHITE()
for o, h, l, c in ohlcv:
...
def apply_up_down_gap_side_by_side_white_lines(series: pl.Series) -> pl.Series:
    ind = qw.CDLGAPSIDESIDEWHITE(14)
...
df = (
    ...
    .lazy()
    .with_columns(
        pl.col("close").map_batches(apply_up_down_gap_side_by_side_white_lines, return_dtype=pl.Float64).alias("up_down_gap_side_by_side_white_lines")
    )
    .collect()
)
```
...
**Implementation**: `quantwave-core/src/indicators/pattern.rs` (`CDLGAPSIDESIDEWHITE` / `CDLGAPSIDESIDEWHITE_METADATA`).
...
**Parity**: `quantwave-core/tests/gold_standard/cdlgapsidesidewhite.json`
