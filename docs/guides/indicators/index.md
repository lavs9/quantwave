...
QuantWave ships **217 native Rust indicators** with bit-identical batch (Polars `.ta()`) and streaming (`Next<T>`) parity — validated by gold-standard vectors and proptests.
...
### Full reference
[Complete catalog](native/) — all 217 indicators by category with slug lookup tables.
