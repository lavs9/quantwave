...
- **`quantwave.datasets` — `load_sample()` and `synthetic()`.** A bundled, deterministic sample OHLCV dataset (`load_sample()`) and a synthetic-series generator (`synthetic()`), so the getting-started guide's "load data, no external file needed" step actually has something to import. `load_sample()`'s data is synthetic (documented on the function itself) — not real NSE or any other market's history.
- **`quantwave.research.conditional_outcome()`** — a statistically honest "given these conditions on my data, how often did this outcome occur" query: point estimate, sample size, a 95% Wilson score confidence interval, a minimum-sample-size guard (refuses to fabricate a number below it), and a chronological first-half/second-half stability split. Takes plain Polars boolean expressions rather than a bespoke query DSL, so it composes directly with existing `.ta`/`.bt` columns.
- **`quantwave.propfirm.simulate_prop_firm_challenge()`** — Monte Carlo-simulates prop-firm evaluation outcomes by block-bootstrap resampling a backtest's closed-trade history (preserving local trade-sequence autocorrelation, unlike single-trade i.i.d. resampling) against a `"static"` or `"trailing"` max-drawdown rule plus a daily-loss limit and profit target. Reports pass probability with a Wilson CI, a breach-reason breakdown, and trades-to-outcome percentiles.

### Changed
- **BREAKING: `portfolio_backtest`'s `signal_type` now defaults to `"weight"` (fraction of total equity) instead of `"shares"` (a literal share count).** A boolean `0/1` entry signal is the most natural way to write "I'm in this name" — under the old `"shares"` default that was a **share count**, so a `1.0` signal on a $1,000,000 book bought one share and deployed almost none of the capital, silently returning close to 0% with no error. `"weight"` matches the convention used by zipline's `order_target_percent`, backtrader's `PercentSizer`, QuantConnect's `SetHoldings`, and vectorbt's `targetpercent`. **Backtests re-run after upgrading without passing `signal_type="shares"` explicitly will report very different — usually much larger, and much more correct — numbers.** A `"target_pct"` mode (normalized across all symbols with a non-zero signal that bar) is also available. See the [Backtest Quickstart](guides/backtest/quickstart.md#4-signal-conventions-get-signal_type-right) for the full convention and a worked example.
...
- **`mama` (MESA Adaptive Moving Average) panicked on a single `NaN` in its input stream.** A `NaN` price propagated unguarded through the recursive Hilbert-transform state until `period.clamp(...)` hit `NaN` bounds, which Rust's `f64::clamp` panics on. A single bad tick from a data vendor should never be able to crash a long-running streaming session; `MAMA::next()` now returns `(NaN, NaN)` for a `NaN` input without touching any internal state, so the next valid price resumes exactly as if the bad tick never happened.
- **`_metadata_generated.py`'s `data_inputs`/`outputs` silently defaulted to `['close']`/`[slug]`** for any indicator with no hand-curated `scripts/metadata_overlay.json` entry — no warning, so most multi-input indicators (e.g. `atr`, which actually needs `high, low, close`) misreported their own signature. Corrected for every indicator whose true runtime arity differs from `['close']`, plus 5 `outputs` name mismatches (`ht_phasor`, `supertrend`, `ttm_squeeze`, `stoch`). Also fixed numeric-default parsing for scientific-notation and boolean Rust defaults (`'1e-5'`, `'true'`/`'false'`), which previously stayed as raw strings.
