...
fn gap_momentum(inputs: &[Series], kwargs: GapMomentumKwargs) -> PolarsResult<Series> {
    ...
    let struct_series =
        StructChunked::from_series("gap_momentum_result".into(), s_o.len(), series_vec.iter())?;
...
fn rodc(inputs: &[Series], kwargs: RodcKwargs) -> PolarsResult<Series> {
    ...
    let mut indicator =
        quantwave_core::RODC::new(kwargs.window_size, kwargs.threshold, kwargs.smooth_period);
    let mut values = Vec::with_capacity(s.len());
...
fn heikin_ashi(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    let out =
        StructChunked::from_series("heikin_ashi_output".into(), s_o.len(), series_vec.iter())?;
    Ok(out.into_series())
}
