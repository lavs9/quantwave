...
def __(mo):
    mo.md(
        ...
        **This is the living specification for "how to use QuantWave ML features correctly"**.
        ...
        - **Batch vs streaming parity + causality** proven (mirrors the Rust proptests in `/tests/test_ml_feature_validation.rs`).
        ...
        - Pure numpy + QuantWave (no sklearn).

        Runnable locally after `maturin develop -p quantwave-py --release && pip install marimo polars numpy`.
    ...
    try:
        ...
        mo.md(
            ...
            **⚠️ Fallback mode — `quantwave` package not found.**
            ...
            This notebook requires `pip install quantwave` (or `maturin develop -p quantwave-py --release` from source). The real feature extractors are not available in the browser environment used by the documentation site.
            """
        )
    ...
    def causality_guard(prices, k=6, HAS_QUANTWAVE=HAS_QUANTWAVE):
        ...
        for t in idxs:
            ...
            results.append(
                {
                    "t": int(t),
                    "cyber_match": abs(cc_val.cycle - cc2.next(float(prices[t])).cycle) < 1e-9
                    if not np.isnan(cc_val.cycle)
                    else True,
                    "hurst_match": abs(hu_val.persistence - hu2.next(float(prices[t])).persistence) < 1e-7
                    if not np.isnan(hu_val.persistence)
                    else True,
                }
            )
        return results
...
def __(mo):
    mo.md(
        ...
        - A 9-feature matrix from these extractors immediately yields a usable tiny model with **clear conditional performance differences** across market regimes — exactly the research goal for feature stability.
        ...
        ```bash
        maturin develop -p quantwave-py --release
