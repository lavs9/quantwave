...
def _():
    mo.md(
        ...
        **Primary cross-epic closure artifact for the ML-features work.**
        ...
        1. From repo root (after any change to backtest bindings): `cd quantwave-py && maturin develop --release`
        ...
        **Note:** This notebook requires the `quantwave` package. It will show a friendly fallback message when viewed on the documentation website.
        ...
        - Python bindings (extractors + new trivial FFI for griffiths + HMM): `quantwave-py/src/lib.rs` + `python/quantwave/__init__.py`
        - Existing patterns: `docs/examples/notebooks/ml_feature_stability.py` (gw7s), `strategy_backtest.py`, `/tests/test_ml_feature_validation.rs`
        - Architecture: `planning/ARCHITECTURE.md`

        This notebook itself is the canonical user-copyable example. All math is zero-lookahead by construction (streaming state machines power both paths).
        """
    )
    return
    ...
    try:
        ...
        mo.md(
            ...
            **⚠️ Fallback mode — `quantwave` package not found.**
            ...
            This is the most important E2E notebook in the repository. It requires a working `quantwave` installation. When viewed on the documentation website, the real feature extractors and backtester are not available.
            """
        )
...
def __(mo):
    mo.md(
        ...
        Timestamps are hourly, starting 2026-05-01 09:30 IST (project convention: IST for all human-facing dates). Only "close" is used by the feature extractors (full OHLCV columns present for realism / future extension).
        """
    )
    return
...
def __(mo):
    mo.md(
        ...
        We run the **exact same** `FeatureExtractor` state machines that power the Rust core, proptests, Polars UDFs, and (future) `.ta().features()`. This produces the identical columns that the locked surface emits:
...
def __(mo):
    mo.md(
        ...
        We use the already-computed feature columns (exactly what `.ta().features().*` would have produced) to derive an `exposure` column via the strategy rules. In a real Rust + Polars pipeline this step is a few chained `.with_columns` after the features calls (or a single custom expr if desired).
...
def __(mo):
    mo.md(
        ...
        Parity tolerances: equity 1e-8, stats 1e-6, trade count exact.
        """
    )
    return
...
def __(mo):
    mo.md(
        ...
        This is the direct analogue of the Rust `FeatureToSignal` (or `RegimeFeaturePAStrategy` pattern in backtest tests). It owns fresh extractor instances and on each `next(bar)` emits a `StrategySignal`-shaped dict with `exposure` + rich `metadata`.
    ...
    mo.md(
        ...
        impl Next<&Bar> for FeatureToSignal {
            ...
            fn next(&mut self, bar: &Bar) -> StrategySignal {
                ...
                let regime_ok = matches!(r, quantwave_core::regimes::MarketRegime::Steady | quantwave_core::regimes::MarketRegime::Bull);
...
def __(mo):
    mo.md(
        ...
        **This is the primary evidence that the 4ps ML features surface + gwx backtester rich metadata + ug9t parity framework integrate cleanly and are ready for users.**
        ...
        — End of notebook (cross-epic deliverable).
        """
    )
    return
